...
### ✅ V1.0 新增功能（2025-11-19）

#### 🔔 自动提醒系统
- [x] 48小时未互动自动标记风险客户
- [x] 系统提醒面板（铃铛图标 + 未读数徽章）
- [x] 4种提醒类型（超期未互动、高机会、高风险、阶段停滞）
- [x] 提醒优先级分级（高/中/低）
- [x] 点击提醒跳转客户详情
- [x] 每10分钟自动检测一次
...
#### 🌡️ 温度自动计算算法
- [x] 智能温度评分（0-100分）
- [x] 4维度评分：基础分 + 阶段评分 + 互动频率 + 时效性 + 情绪评分
- [x] 温度等级自动分类（热/温/中性/冷）
- [x] 客户详情页「重新计算温度」按钮
- [x] 仪表盘「更新温度」按钮
- [x] 每天凌晨2点自动更新所有客户温度
...
#### 🎯 风险/机会自动识别
- [x] 智能高机会客户识别（温度≥80、高转化阶段、高频互动、正向情绪）
- [x] 智能高风险客户识别（温度<40、超期未互动、阶段停滞、负向情绪、互动下降）
- [x] 仪表盘「智能评估」按钮
- [x] 自动创建提醒（高风险/高机会）
- [x] 每天凌晨3点自动评估
...
#### 📱 移动端响应式优化
- [x] 完整的移动端响应式布局
- [x] 三档响应式断点（Desktop/Tablet/Mobile）
- [x] 导航栏移动端优化（Icon-only按钮）
- [x] 看板列宽度自适应
- [x] KPI卡片响应式网格（4列→2列→1列）
- [x] Modal宽度适配屏幕（95vw）
- [x] 触摸目标优化（最小44x44px）
...
### 生产环境（V1.0）
```
https://0c52435e.crm-high-trust-sales.pages.dev
```
...
### v1.0.0 (2025-11-19) 🎉
**V1.0 重大功能发布：**
- ✨ 自动提醒系统（48小时未互动自动标记风险）
- ✨ 温度自动计算算法（0-100分智能评分）
- ✨ 风险/机会自动识别（智能标记高价值客户）
- ✨ 拖拽式看板（HTML5 Drag & Drop）
- ✨ 移动端响应式优化（3档响应式断点）
